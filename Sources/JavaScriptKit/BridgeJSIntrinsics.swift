...
public protocol _BridgedSwiftOptionalScalarBridge: _BridgedSwiftTypeLoweredIntoSingleWasmCoreType,
    _BridgedSwiftStackType where StackLiftResult == Self
{
...
/// Generic stack-based ABI for `Optional<T>` where `T` already conforms to
/// `_BridgedSwiftStackType`.  Uses **conditional push/pop**: the payload is
/// pushed/popped only when the value is non-nil.
///
/// This replaces the former `_BridgedSwiftGenericOptionalStackType` marker
/// protocol and unifies all Optional stack handling under one convention.
...
// Generic stack ABI delegation: JSUndefinedOr → Optional
