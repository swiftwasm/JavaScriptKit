...
//
// The per-type ABI conformances for Bool/Int*/UInt*/Float/Double used to live here. They are
// now generated into `Generated/BridgeJSIntrinsics+ABI.swift` from the ABI description in
// `Plugins/BridgeJS/Sources/BridgeJSSkeleton/BridgeJSABI.swift`, which is the same description
// BridgeJSLink generates the JavaScript half from -- so the two sides of the boundary cannot
// drift apart. Regenerate with `./Utilities/bridge-js-generate.sh`.
//
// What remains hand-written below is what a layout cannot describe: the irreducible codec
// leaves (String, JSObject, JSValue), the generic container defaults that Swift composes at
// runtime via protocol dispatch, and the closure/typed-array machinery.
