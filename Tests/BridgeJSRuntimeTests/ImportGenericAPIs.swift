...
@JSFunction func jsGenericAfterOptionalArray<T: BridgedSwiftGenericBridgeable>(
    _ values: [Int]?,
    _ value: T
...
@JSFunction func jsGenericThrowOrRoundTrip<T: BridgedSwiftGenericBridgeable>(
    _ shouldThrow: Bool,
    _ value: T
) throws(JSException) -> T

@JSClass struct ImportGenericBoxed {
...
@Suite struct ImportGenericAPITests {
    ...
    /// A generic parameter shares the stacks with any other stack-lowered
    /// parameter, so both have to arrive in declaration order.
    @Test func genericAlongsideOptionalArray() throws {
        #expect(try jsGenericAfterOptionalArray([1, 2], 42) == "[1,2]|42")
        #expect(try jsGenericAfterOptionalArray([1, 2], GenericRTPoint(x: 3, y: 4)) == #"[1,2]|{"x":3,"y":4}"#)
        #expect(try jsGenericAfterOptionalArray(nil, "x") == #"null|"x""#)
    }

    @Test func genericImportPropagatesJSException() throws {
        #expect(try jsGenericThrowOrRoundTrip(false, 42) == 42)
        #expect(try jsGenericThrowOrRoundTrip(false, GenericRTPoint(x: 1, y: 2)).x == 1)
        do {
            let _: Int = try jsGenericThrowOrRoundTrip(true, 0)
            Issue.record("Expected exception")
        } catch {
            #expect(error.description.contains("TestError"))
        }
        // A throwing generic call must not strand its argument on the shared
        // stack: the next call has to read its own value back.
        #expect(try jsGenericThrowOrRoundTrip(false, GenericRTPoint(x: 7, y: 8)).y == 8)
    }

    @Test func importGenericConstructor() throws {
        let boxedInt = try ImportGenericBoxed(42)
        #expect(try boxedInt.unwrap() == 42)
        let boxedText = try ImportGenericBoxed("boxed")
        #expect(try boxedText.unwrap() == "boxed")
        let boxedPoint = try ImportGenericBoxed(GenericRTPoint(x: 1, y: 2))
        let point: GenericRTPoint = try boxedPoint.unwrap()
        #expect(point.x == 1)
        #expect(point.y == 2)
    }
