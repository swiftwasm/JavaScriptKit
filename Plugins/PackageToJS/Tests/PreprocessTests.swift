...
@Suite struct PreprocessTests {
    @Test func thenBlock() throws {
        ...
        /* #endif */
        """
        let options = PreprocessOptions(variables: ["FOO": true])
    ...
    @Test func elseBlock() throws {
        ...
        /* #endif */
        """
        let options = PreprocessOptions(variables: ["FOO": false])
        let result = try preprocess(source: source, options: options)
        #expect(result == "\nconsole.log(\"BAR\");\n")
    }
    ...
    @Test func undefinedVariable() throws {
        ...
        /* #endif */
        """
        let options = PreprocessOptions(variables: [:])
    ...
    @Test(arguments: [
        (foo: true, bar: true, expected: "\nconsole.log(\"FOO\");\n\nconsole.log(\"FOO & BAR\");\n\n"),
        (foo: true, bar: false, expected: "\nconsole.log(\"FOO\");\n\nconsole.log(\"FOO & !BAR\");\n\n"),
        (foo: false, bar: true, expected: "\nconsole.log(\"!FOO\");\n"),
        (foo: false, bar: false, expected: "\nconsole.log(\"!FOO\");\n"),
    ])
    func nestedIfDirectives(foo: Bool, bar: Bool, expected: String) throws {
        ...
        /* #endif */
        """
        let options = PreprocessOptions(variables: ["FOO": foo, "BAR": bar])
        let result = try preprocess(source: source, options: options)
