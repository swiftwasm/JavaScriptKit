...
// A generic parameter alongside another parameter that pushes onto the shared
// stacks: both are lowered in reverse declaration order.
@JSFunction func importGenericAfterOptionalArray<T: BridgedSwiftGenericBridgeable>(
    _ values: [Int]?,
    _ value: T
) throws(JSException) -> T

@JSClass struct GenericPairFactory {
    @JSFunction init<T: BridgedSwiftGenericBridgeable, U: BridgedSwiftGenericBridgeable>(
        _ tag: String,
        _ first: T,
        _ second: U
    ) throws(JSException)
